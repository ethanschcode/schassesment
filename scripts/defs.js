...
Defs.turrets.Laser = {
	...
	shoot: function (creeps) {
		const audioElement = new Audio("laserfx.mp3");
...
Defs.turrets.Cannon = {
	...
	cell: 0,
	shoot: function (creeps) {
		const audioElement = new Audio("cannonfx.mp3");
		audioElement.volume = 0.1;
		audioElement.play()
		var creep = creeps[Math.rand(creeps.length - 1)];
		var cell = this.cell % 4;
		var cannon = { x: this.x + (cell % 2 === 0 ? -5 : 5), y: this.y + (cell < 2 ? -5 : 5) };
		var turret = this;
		...
		game.run.push({ what: function () {
			...
			if (Math.move(cannon, creep, 3)) {
				...
				canvas.fillRect(cannon.x - 2, cannon.y - 2, 4, 4);
			}
		}, until: Infinity });
...
Defs.turrets.Landmine = {
	...
	shoot: function (creeps) {
		const audioElement = new Audio("landminefx.mp3");
...
Defs.turrets.Mortar = {
	...
	shoot: function (creeps) {
		const audioElement = new Audio("mortarfx.mp3");
		audioElement.volume = 0.7;
		audioElement.play()
		var creep = creeps[0];
		var turret = this;
